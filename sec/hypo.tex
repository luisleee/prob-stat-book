...
\[
\Dev[\overline{D}] =\Dev[\overline{X}]+\Dev[\overline{Y}]= \frac{\sigma_1^2}{n_1}+\frac{\sigma_2^2}{n_2}
\]

但在成对数据中，$X$和$Y$并没有预设的独立性。于是$X,Y$协方差仍然需要考虑。
\[
\Dev[\overline{D}]=\frac{\sigma_1^2}{n_1}+\frac{\sigma_2^2}{n_2}-2\Cov(\overline{X},\overline{Y})
\]
