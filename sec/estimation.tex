...
\[
\Dev[\hat{\theta}_1] \leq \Dev[\hat{\theta}_2]
\]
...
对于连续的情况，也有类似的结论成立。$X_i$落在$[x,x+\dif x]$内的概率质量为$f(x;\theta)\dif x$，则$X_1\times\cdots\times X_n$落在$[x_1,x_1+\dif x_1]\times\cdots\times[x_n,x_n+\dif x_n]$内的概率质量就是$ \prod_{i=1}^nf(x_i;\theta)\dif x_i=\left[\prod_{i=1}^nf(x_i;\theta)\right]\dif\mathbf{x}$，即联合分布在$\mathbf{x}$处的概率密度为$\prod_{i=1}^nf(x_i;\theta)$。不过这都是废话，在测度视角下看二者没有本质区别。
