...
$\mathbf{Z}\sim N_n(Q\boldsymbol{\mu},Q\sigma^2I_nQ^T) = N_n(Q\boldsymbol{\mu},\sigma^2I_n)$。它的各分量是独立的！
...
& = \sum_{i=1}^r n_i E(\overline{X}_i^2) - nE(\overline{X}^2)\\
& = \sum_{i=1}^r n_i [D(\overline{X}_i)+(E(\overline{X}_i))^2] - n[D(\overline{X})+(E(\overline{X}))^2]\\
