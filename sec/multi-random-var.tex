...
以二元随机变量\((X,Y)\)为例，\(X\)与\(Y\)各自是\(\Omega\to \mathbb{R}\)和\(\Omega\to \mathbb{R}\)的函数。我们就可以粘合出一个二元随机变量，也就是\(X\times Y: \Omega \to \mathbb{R}^2\)。

像一元随机变量那样，我们也可以计算$n$元随机变量$\mathbf{X}$的取值落在$\mathbb{R}^n$中一个可测子集$S$内的概率$P(\mathbf{X}\in S)$。
...
\[
P(\mathbf{X}\in S)=\int_{S}f_{\mathbf{X}}(\mathbf{x})\dif\mathbf{x}
\]
...
\[
f_{X_1}(x)=\int_{\pi_1(\mathbf{x})=x}f_{\mathbf{X}}(\mathbf{x})\dif\mathbf{x}
\]
...
&= \iint_{x+y\leq z}f_{XY}(x,y)\dif x\dif y\\
&= \int_{-\infty}^{\infty}\left[\int_{-\infty}^{z-y}f_{XY}(x,y) \dif x\right]\dif y\\
&= \int_{-\infty}^{\infty}\left[\int_{-\infty}^{z}f_{XY}(u-y,y) \dif u\right]\dif y&\text{u=z-y}\\
&= \int_{-\infty}^{z}\left[\int_{-\infty}^{\infty}f_{XY}(u-y,y) \dif y\right]\dif u &\text{Fubini定理}\\
f_{X+Y}(z)&=\frac{\dif F_{X+Y}(z)}{\dif z} = \int_{-\infty}^{\infty}f_{XY}(z-t,t) \dif t &\text{微积分基本定理}
\end{align*}
...
\begin{align*}
F_{Z}(z) &= P(XY \leq z) \\
&= \iint_{xy \leq z} f_{XY}(x,y) \,\dif x\,\dif y \\
&= \int_{-\infty}^{0} \left[ \int_{z/x}^{\infty} f_{XY}(x,y) \,\dif y \right] \dif x 
   + \int_{0}^{\infty} \left[ \int_{-\infty}^{z/x} f_{XY}(x,y) \,\dif y \right] \dif x \\
&= \int_{-\infty}^{0} \left[ \int_{-\infty}^{z/x} f_{XY}(x,y) \,\dif y \right] \dif x 
   + \int_{0}^{\infty} \left[ \int_{-\infty}^{z/x} f_{XY}(x,y) \,\dif y \right] \dif x \\
&= \int_{-\infty}^{\infty} \left[ \int_{-\infty}^{z/x} f_{XY}(x,y) \,\dif y \right] \dif x \\
&= \int_{-\infty}^{\infty} \left[ \int_{-\infty}^{z} f_{XY}\left(x, \frac{u}{x}\right) \frac{1}{|x|} \,\dif u \right] \dif x & u = xy \\
&= \int_{-\infty}^{z} \left[ \int_{-\infty}^{\infty} f_{XY}\left(x, \frac{u}{x}\right) \frac{1}{|x|} \,\dif x \right] \dif u & \text{Fubini 定理} \\
f_{Z}(z) &= \frac{\dif F_{XY}(z)}{\dif z} = \int_{-\infty}^{\infty} f_{XY}\left(x, \frac{z}{x}\right) \frac{1}{|x|} \,\dif x & \text{微积分基本定理}
\end{align*}

除法是类似的，令$Z=Y/X$：
\[
f_Z(z)= \int_{-\infty}^{\infty} f_{XY}(x, zx) |x|\dif x
\]
...
\[
\int_{g^{-1}([a,b])} f(\mathbf{x})\,\dif\mathbf{x}
=
\int_{t=a}^{b}
\left[
...
\[
\dif x = J_\Phi(y_1,\ldots,y_{n-1}, t)\, \dif y_1 \cdots \dif y_{n-1} \dif t
\]
按照隐函数定理，Jacobi因子正好为$|\nabla g(\mathbf{x})|$，即

\[
\dif\mathbf{x} = \frac{1}{|\nabla g(\mathbf{x})|} d\sigma(\mathbf{x}) \dif t
\]

于是有
\[
\int_{\Omega} f(x)\,\dif\mathbf{x}
= \int_{t = a}^{b}
  \left[ \int_{M_t} \frac{f(\mathbf{x})}{|\nabla g(\mathbf{x})|}\, d\sigma(\mathbf{x}) \right] \dif t
\]
...
\[
    F_Y(y) = \int_{\Omega} f(x)\,\dif\mathbf{x}
= \int_{-\infty}^{y}
...
$f_Y(y) = \frac{\dif F_Y(y)}{\dif y} = \int_{g(\mathbf{x})=t} \frac{f(\mathbf{x})}{|\nabla g(\mathbf{x})|}\, d\sigma(\mathbf{x})$
...
\[
\int_{g^{-1}([a,b])} f(x)\, \dif x = \int_{t = a}^{b} \left( \sum_{x \in g^{-1}(t)} \frac{h(x)}{|g'(x)|} \right) \dif t
\]
